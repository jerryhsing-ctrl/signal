...
#else
    (void)core_id;
    std::cerr << "Thread pinning is only supported on Linux builds." << std::endl;
    return false;
#endif
}
...
#else
    pthread_setname_np(short_name.c_str());
#endif
}
...
inline bool set_realtime_priority() {
    ...
    return true;
#else
    std::cerr << "Realtime scheduler promotion is only supported on Linux builds." << std::endl;
    return false;
#endif
}
