...
class GroupRank:
    ...
    def get_rank(self, name: str) -> int:
        ...
        if name not in self._name_to_gain:
            return -1
        rank = 1
        for _, n in sorted(self._gain_to_name.items(), reverse=True):
            if n == name:
                return rank
            rank += 1
        return -1
    ...
    def is_top_n(self, name: str, n: int) -> bool:
        if name not in self._name_to_gain:
            return False
        count = 0
        for _, nm in sorted(self._gain_to_name.items(), reverse=True):
            if nm == name:
                return True
            count += 1
            if count >= n:
    ...
    def iter_ranked(self) -> list[tuple[float, str]]:
        ...
        return sorted(self._gain_to_name.items(), reverse=True)
